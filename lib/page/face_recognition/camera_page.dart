...
class _FaceScanScreenState extends State<FaceScanScreen> {
  ...
  Future<void> detectFacesFromImage(CameraImage image) async {
    ...
    // Assuming we're working with the first plane of the image (plane[0])
    final bytesPerRow = image.planes[0].bytesPerRow;

    // Create an InputImage from CameraImage
    final inputImageMetadata = InputImageMetadata(
      rotation: imageRotation,
      format: InputImageFormat.yuv420, // Update the format as needed
      size: Size(image.width.toDouble(), image.height.toDouble()),
      bytesPerRow: bytesPerRow, // Add bytesPerRow here
    );

    final inputImage = InputImage.fromBytes(
      bytes:
          image.planes[0].bytes, // Assuming plane[0] holds the image byte data
      metadata: inputImageMetadata, // Pass metadata
    );
  ...
  void initState() {
    ...
    _faceDetector = GoogleMlKit.vision.faceDetector(
      FaceDetectorOptions(
        enableContours: true, // Enable contour detection (optional)
        enableClassification: true, // Enable classification (optional)
        performanceMode: FaceDetectorMode
            .accurate, // Use 'accurate' mode for better detection
        enableTracking: true, // Enable face tracking if required
      ),
    );
