...
class MLService {
  ...
  imglib.Image _cropFace(CameraImage image, Face faceDetected) {
    ...
    // Calculate the crop coordinates (with a 10-pixel margin)
    int x = (faceDetected.boundingBox.left - 10).round();
    int y = (faceDetected.boundingBox.top - 10).round();
    int width = (faceDetected.boundingBox.width + 10)
        .round(); // Adding 10 on both sides
    int height = (faceDetected.boundingBox.height + 10)
        .round(); // Adding 10 on both sides
    // Crop the image based on the calculated coordinates
    return imglib.copyCrop(
      convertedImage,
      x: x, // x coordinate for the crop
      y: y, // y coordinate for the crop
      width: width, // width of the cropped area
      height: height, // height of the cropped area
    );
    // return imglib.copyCrop(
